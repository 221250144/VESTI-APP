...
| 路由 | 上游 | 说明 |
|---|---|---|
| `POST /v1/chat/completions` | deepseek / moonshot(池化) | 按 model 前缀路由:`moonshot*`/`kimi*` → Kimi,其余 → DeepSeek 池。**无白名单**,model id 原样透传 |
| `POST /api/chat` | 同上 | 旧客户端兼容(强制非流式) |
| `POST /api/embeddings` `/v1/embeddings` | 阿里百炼 DashScope | 服务端统一选型(默认 `text-embedding-v4`),客户端不传 model;实际模型经 `x-proxy-model-used` 响应头告知 |
...
| `GET /health` | — | 探活 + 各 provider 池内 key 数量/端点名 |

## key 池与兜底

每个供应商是一个**上游条目池**:请求按轮询顺序起步,遇到连接失败/超时或
401/402/403/408/429/5xx 自动 failover 到下一条,响应头
`x-proxy-provider-used` / `x-proxy-fallback-reason` 记录实际落点与 failover 原因。

- DeepSeek 池 = 官方 `DEEPSEEK_API_KEY(S)` + 阿里百炼 token-plan `ALI_BAILIAN_CHAT_KEY(S)`(兜底;
  百炼模型命名不同,`deepseek-v4-flash → deepseek-v4-flash-0731`、`deepseek-chat → 同上`、
  `deepseek-reasoner → deepseek-v4-pro`,其余原样透传)
- Embedding 池 = 阿里百炼 DashScope `ALI_BAILIAN_EMBED_KEY(S)`,模型由 `VESTI_EMBEDDING_MODEL` 指定(默认 text-embedding-v4)
- 逗号分隔可加多条 key:`DEEPSEEK_API_KEYS=k1,k2`(旧的单数变量继续有效,自动并入池)
...
ALI_BAILIAN_CHAT_KEY=<阿里百炼 token-plan chat key(兜底)>
ALI_BAILIAN_EMBED_KEY=<阿里百炼 DashScope embedding key>
EOF
...
- 限流:chat 600 次/10 分钟/IP,绘图 60 次/10 分钟/IP,embeddings 240 次/10 分钟/IP,众筹兑换 20 次/小时/IP(进程内滑窗)
