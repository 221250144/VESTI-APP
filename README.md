...
- App 首次启动时需要在本机注册账号；注册成功会立即赠送 3 个自然月的 Beta 会员，并自动登录。
- 只有已登录且会员未到期的账号才能进入主界面；会话采集、扩展桥、分析、Agent 与悬浮球等后台能力也受同一会员门禁保护。
- 当前版本是单设备、单账号方案。密码不会明文保存，而是以随机盐和 `scrypt` 哈希写入 Electron 用户数据目录中的 `membership.json`；登录会话在重启后保留。
- 本地会员门禁用于 Beta 阶段验证产品流程，并不等同于商业防篡改授权。正式收费前应将账号、会员有效期和续费校验迁移到服务端，并由服务端签发可短期离线验证的授权凭证。
- 服务端实现任务、数据库和接口契约见 [会员与登录服务端实施规格](docs/membership-auth-server-spec.md)。
