...
import { hostFromPath, rewriteSessionIdForHost } from '../src/platform/PathResolver.js';
...
describe('UNC path helpers', () => {
  ...
  it('derives wsl:<distro> hosts from UNC session paths', () => {
    expect(hostFromPath('\\\\wsl$\\Ubuntu\\home\\alice\\.codex\\sessions\\x.jsonl')).toBe('wsl:ubuntu');
    expect(hostFromPath('\\\\wsl.localhost\\Debian\\root\\.kimi\\sessions\\h\\u\\wire.jsonl')).toBe('wsl:debian');
    // forward-slash tolerant
    expect(hostFromPath('//wsl$/Ubuntu/home/alice/.claude/projects/p/s.jsonl')).toBe('wsl:ubuntu');
...
describe('session id disambiguation', () => {
  ...
  it('prefixes WSL session ids with wsl-<distro>-', () => {
    expect(rewriteSessionIdForHost('abc-123', 'wsl:Ubuntu')).toBe('wsl-Ubuntu-abc-123');
  });
  ...
  it('stores the same sessionId from native and WSL as two distinct sessions', async () => {
    ...
    const wsl = { ...base, host: 'wsl:Ubuntu', sessionId: rewriteSessionIdForHost(base.sessionId, 'wsl:Ubuntu') };
    ...
    expect(wslConverted.session.id).toBe('codex:wsl-Ubuntu-shared-session');
    expect(wslConverted.session.host).toBe('wsl:Ubuntu');
    // Messages follow the rewritten session id so the chains never mix.
    expect(wslConverted.messages[0].sessionId).toBe('codex:wsl-Ubuntu-shared-session');
    ...
    expect(db.getWorkSession('codex:wsl-Ubuntu-shared-session')?.host).toBe('wsl:Ubuntu');
