...
import { GENERATED_CATALOG } from "./catalog";
...
export interface CuratedPrompt {
  ...
  /** Optional one-liner: what the prompt is for / when to use it (适用场景). */
  description?: { en: string; zh: string };
  /** Optional search/grouping keywords; matched by searchCuratedPrompts. */
  tags?: string[];
...
const BASE_PROMPTS: CuratedPrompt[] = [
  {
    id: "expert-explainer",
...
// Full catalog = hand-curated essentials + the generated by-category catalog
// + the in-house curated collections (deduped by id, first occurrence wins).
// Order: essentials first, then the supermarket categories, then collections.
/**
 * Merge catalog lists by id, keeping the first occurrence. Idempotent by
 * construction: merging the same list again (e.g. a repeated seed pass) is a
 * no-op — no duplicate ids can survive.
 */
export function mergeCuratedPrompts(...lists: CuratedPrompt[][]): CuratedPrompt[] {
  const seen = new Set<string>();
  const merged: CuratedPrompt[] = [];
...
export const CURATED_PROMPTS: CuratedPrompt[] = mergeCuratedPrompts(
  BASE_PROMPTS,
  GENERATED_CATALOG,
  CURATED_COLLECTIONS,
);

/** Ordered list of big-categories present in the catalog (for the 超市). */
export function getCuratedCategories(locale: "en" | "zh"): string[] {
  const lang: "en" | "zh" = locale === "zh" ? "zh" : "en";
  const order: string[] = [];
  const seen = new Set<string>();
  for (const p of CURATED_PROMPTS) {
    const label = p.category[lang];
    if (!seen.has(label)) {
      seen.add(label);
      order.push(label);
    }
...
/** Resolve the whole catalog to a single locale (no daily rotation). */
export function resolveCuratedPrompts(locale: "en" | "zh"): PlazaPromptResolved[] {
  const lang: "en" | "zh" = locale === "zh" ? "zh" : "en";
  return CURATED_PROMPTS.map((p) => ({
    id: p.id,
...
/**
 * Search the curated catalog by trigger/keyword (title, body, category, tags).
 * Used by the in-dock 提示词助手 so its search also surfaces 优质提示词.
 */
export function searchCuratedPrompts(
  query: string,
  locale: "en" | "zh",
  limit = 6,
): PlazaPromptResolved[] {
  const all = resolveCuratedPrompts(locale);
