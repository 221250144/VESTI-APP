export const jaTranslations = {
  ...
  settings: {
    ...
    modelAccess: {
      ...
      privacyDisclosure: "キャプチャ・ローカル保存・キーワード検索は 100% デバイス内で完結します。AI 機能（要約・探索・埋め込み・プロンプトツール・円卓）は、関連する会話テキストを設定済みのモデルに送信します。既定では Vesti のプロキシ経由でルーティングされ、実際のプロバイダーとモデルはプロキシサービスが選択します。独自の API キーを追加すると、カスタムの OpenAI 互換エンドポイントへ直接接続できます。",
  ...
  insights: {
    ...
    recapTitle: "今週のふりかえり",
    statConversations: "対話数",
    statActiveDays: "アクティブ日数",
    statStreak: "継続週数",
    statTopPlatform: "よく使ったプラットフォーム",
